...
Initial_yes=[];
Initial_no=[];
for ii=1:length(rSac)
    if rSac(ii)>=25 & rSac(ii)<=75
        Initial_yes=[Initial_yes;class(ii) rSac(ii) n(ii) ];

    else
        Initial_no=[Initial_no;class(ii)  rSac(ii) n(ii) ];
    end    
end
%%%%%%%Last 4:two Matrixs one between 25 to 75 other out of%%%%%%%%
final_yes=[];
final_no=[];
for ii=1:length(rSac5)
    if rSac5(ii)>=25 & rSac5(ii)<=75
        final_yes=[final_yes;class5(ii)  rSac5(ii) n5(ii) ];
    else
        final_no=[final_no;class5(ii)   rSac5(ii)  n5(ii) ];
    end
end
...
[pa,ha,statsa] = ranksum(class1n(:,2),class1nf(:,2));
[p2a,h2a,stats2a] = ranksum(class2n(:,2),class2nf(:,2));
[p3a,h3a,stats3a] = ranksum(class3n(:,2),class3nf(:,2));
[p4a,h4a,stats4a] = ranksum(class4n(:,2),class4nf(:,2));
[p5a,h5a,stats5] = ranksum(class5n(:,2),class5nf(:,2));
%[p6a,h6a,stats6a] = ranksum(class6n(:,2),class6nf(:,2));
[p7a,h7a,stats2a] = ranksum(class7n(:,2),class7nf(:,2));
[p8a,h8a,stats8a] = ranksum(class7n(:,2),class7nf(:,2));
 %psa=[pa p2a p3a p4a p5a p6a p7a p8a];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[p,h,stats] = ranksum(class1(:,2),class1b(:,2));
%%%%%%%%wilcox rank sum of each class%%%%%%%%%%%%%%%%%%
[p2,h2,stats2] = ranksum(class2(:,2),class2b(:,2));
[p3,h3,stats3] = ranksum(class3(:,2),class3b(:,2));
[p4,h4,stats4] = ranksum(class4(:,2),class4b(:,2));
[p5,h5,stats5] = ranksum(class5(:,2),class5b(:,2));
[p6,h6,stats6] = ranksum(class6(:,2),class6b(:,2));
[p7,h7,stats2] = ranksum(class7(:,2),class7b(:,2));
[p8,h8,stats8] = ranksum(class7(:,2),class7b(:,2));
%%%%%%%%%%%%%%%%%pvalues%%%%
 ps=[p p2 p3 p4 p5 p6 p7 p8];
figure(1)
hold on
a=scatter(bd,rSac,"b","*")
...
ylabel("Percentages to Targets of Righthand")
yticks([0 50 100])
%%%%%%%%only P-values%%%%%%%%%%%%%%%%%%%%%%%%%
two='-2**-\np=0.0005';
three='-3*-\np=0.05';
